...
switch lower(Wp.name)
    ...
    case lower('2turb_demo')
        ...
        % Format inputs and delta inputs
        for j = 1:600
            input{j}.beta  = [0.4; 0.5];
            input{j}.dbeta = [0.0; 0.0];
            if j < 200
                input{j}.phi   = [0.0; 0.0];
            else
                input{j}.phi   = [25.0; 0.0];
            end;
            input{j}.dphi  = [0.0; 0.0];
        end;
        ...
        Wp.Turbulencemodel  = 'WFSim3'; % Turbulence model
