function [Sm,dSm,Ueffect,Ur,a,Power,CT,CP,phi,Fxmean,Fymean,Umean,Vmean,Umatrix] = Actuator_Joeri(Wp,u,v,Phi,beta,dbeta,dPhi)
...
for kk=1:Wp.N
    ...
    dUdv(kk,:)          =  vv./(U(kk,1:length(Wp.yline{kk})));
    dUdU(kk,1:length(Wp.yline{kk}))          =  u(Wp.xline(kk,:),Wp.yline{kk})./(U(kk,1:length(Wp.yline{kk})));
    phi(kk)       = 0;%-180/pi*atan(mean(v(Wp.xline(kk,:)-5,Wp.yline{kk}))/mean(u(Wp.xline(kk,:)-5,Wp.yline{kk}))); %% added an offset here
    Ue(kk,:)       =cos(atan(vv./(u(Wp.xline(kk,:),Wp.yline{kk})))+Phi(kk)/180*pi).*U(kk,1:length(Wp.yline{kk}));
    
    uu=u(Wp.xline(kk,:),Wp.yline{kk});
    dUedv(kk,:)=-sin(atan(vv./(u(Wp.xline(kk,:),Wp.yline{kk}))) + Phi(kk)/180*pi)./((u(Wp.xline(kk,:),Wp.yline{kk})).*(vv.^2./(u(Wp.xline(kk,:),Wp.yline{kk})).^2 + 1)).*U(kk,1:length(Wp.yline{kk}))+cos(atan(vv./(u(Wp.xline(kk,:),Wp.yline{kk})))+Phi(kk)/180*pi).*dUdv(kk,:); %% diff Ue wrt v
    dUedv(kk,:)=-uu.*sin(atan(vv./uu) + Phi(kk)/180*pi)./(vv.^2+uu.^2).*U(kk,1:length(Wp.yline{kk}))+cos(atan(vv./(u(Wp.xline(kk,:),Wp.yline{kk})))+Phi(kk)/180*pi).*dUdv(kk,:); %% diff Ue wrt v
    dUedU(kk,:)=(vv.*sin(atan(vv./(u(Wp.xline(kk,:),Wp.yline{kk}))) + Phi(kk)/180*pi))./((vv.^2 + (u(Wp.xline(kk,:),Wp.yline{kk})).^2)).*U(kk,1:length(Wp.yline{kk}))+cos(atan(vv./(u(Wp.xline(kk,:),Wp.yline{kk})))+Phi(kk)/180*pi).*dUdU(kk,1:length(Wp.yline{kk})); %% diff Ue wrt u
    dUedPhi(kk,:)=-1/180*pi*sin(atan(vv./(u(Wp.xline(kk,:),Wp.yline{kk}))) + Phi(kk)/180*pi).*U(kk,1:length(Wp.yline{kk})); %% diff wrt to Ue
    ...
    Fthrust             = 1/2*Rho*CT(kk)*(Ue(kk,:)*(beta(kk)+1)).^2; % similar as 1/2*Rho*CT(kk)*(U/(1-a(kk))).^2; %somethin to try 2*60*ones(1,size(U(kk,1:length(Wp.yline{kk})),2));%
    ...
    dSm.xdu((Wp.xline(kk,:)-3)*(Wp.Ny-2)+Wp.yline{kk}-1,(Wp.xline(kk,:)-3)*(Wp.Ny-2)+Wp.yline{kk}-1) = diag((-dFxdU.*dUedU(kk,1:length(Wp.yline{kk})).*Wp.dyy2(1,Wp.yline{kk})));
    ...
    Power2(kk)     = 1/F*mean(2*Rho*pi*(0.5*Wp.turbine.Drotor)^2*beta(kk)*Ue(kk,:).^3); %% JW ??
    Power(kk)     = 1/F*2*Rho*pi*(0.5*Wp.turbine.Drotor)^2*beta(kk)*mean(Ue(kk,:)).^3; %% JW ??
    
    %% Derivative cost function with respect to state and beta
    JXXu(Wp.xline(kk,:)-2,Wp.yline{kk}-1) =  -1/F*pi*(0.5*Wp.turbine.Drotor)^2*6*Rho*beta(kk).*mean(Ue(kk,:)).^2.*dUedU(kk,1:length(Wp.yline{kk}))./(length(Wp.yline{kk}));%-3*mean(urotor).^2*pi*(0.5*Wp.turbine.Drotor)^2*2*beta(kk)/(length(solind)/2);
    ...
    JXXv(Wp.xline(kk,:)-1,Wp.ylinev{kk}-2) = -1/F*pi*(0.5*Wp.turbine.Drotor)^2*6*Rho*beta(kk).*mean(Ue(kk,:)).^2.*dUedv(kk,:)./((length(Wp.yline{kk})))*dvvdv;%-3*mean(urotor).^2*pi*(0.5*Wp.turbine.Drotor)^2*2*beta(kk)/(length(solind)/2);
    
    %% two lines above should be checked!!
    
    dSm.dJdx  = [vec(JXXu');vec(JXXv');vec(JXXp')];
    dSm.dJdbeta(kk)   = -1/F*pi*(0.5*Wp.turbine.Drotor)^2*(2*Rho).*mean(Ue(kk,:)).^3;
    dSm.dJdPhi(kk)  = -1/F*6*Rho*pi*(0.5*Wp.turbine.Drotor)^2*beta(kk)*mean(Ue(kk,:)).^2.*mean(dUedPhi(kk,:));
    
    
    %% Input to Ax=b
    Sm.x(Wp.xline(kk,:)-2,Wp.yline{kk}-1)      = -Fx'.*Wp.dyy2(1,Wp.yline{kk})'*beta(kk)/beta(kk);
    Sm.dx(Wp.xline(kk,:)-2,Wp.yline{kk}-1)     = -dFx'.*Wp.dyy2(1,Wp.yline{kk})'*dbeta(kk);   %dSm.dbeta * dbeta
    
    Sm.y(Wp.xline(kk,:)-1,Wp.yline{kk}(2:end)-2)      = scale*Fy(2:end)'.*Wp.dyy2(1,Wp.yline{kk}(2:end))'*beta(kk)/beta(kk);
